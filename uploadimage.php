...
if(isset($_POST["submit"])) {
  ...
  $imgdesc =mysqli_real_escape_string($conn,$_POST["imgdesc"]);
...
if($flag == 1) {
 $sql = "INSERT INTO holiday_images (imgname,imgdesc,imgpath) 
                            VALUES ('$imgname','$imgdesc','$imgpath')";
                            //echo $sql;
                        if(($conn->query($sql))== true)
                        {                       
                              echo "Uploaded Successfully<br>";
                              header('location:imageuploadstatus.php?status=success');
                        }
                        else{
